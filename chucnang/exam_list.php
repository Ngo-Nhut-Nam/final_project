...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function filterExams() {
        ...
        exams.forEach(exam => {
            const examSubject = exam.dataset.subject.trim().toLowerCase();
            const examType = exam.dataset.type.trim().toLowerCase();

            // Kiểm tra khớp môn học và loại đề
            const matchSubject = (selectedSubject === "all") || (examSubject === selectedSubject);
            const matchType = (selectedType === "all") || (examType === selectedType);

            // Hiển thị card nếu khớp, ẩn nếu không
            exam.style.display = (matchSubject && matchType) ? "" : "none";
        });
    }

    // Gắn sự kiện cho cả 2 filter
